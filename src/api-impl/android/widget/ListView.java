...
public class ListView extends AbsListView {
	...
	public ListView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		if (a.hasValue(R.styleable.ListView_dividerHeight)) {
	...
	public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate) {
		...
		final int fadingEdge = 0; // getVerticalFadingEdgeLength();
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		if (getChildCount() > 0) {
			...
			if (focusedChild != null) {
				...
				final int offset = Math.max(0, childBottom - (h - paddingTop));
	...
	private void correctTooHigh(int childCount) {
		...
		if (lastPosition == mItemCount - 1 && childCount > 0) {
			...
			if (bottomOffset > 0 && (mFirstPosition > 0 || firstTop < mListPadding.top)) {
				...
				for (int i = getChildCount() - 1; i >= 0; i--) {
					getChildAt(i).offsetTopAndBottom(bottomOffset);
				}
				if (mFirstPosition > 0) {
		...
		if (mFirstPosition == 0 && childCount > 0) {
			...
			if (topOffset > 0) {
				if (lastPosition < mItemCount - 1 || lastBottom > end) {
					...
					for (int i = getChildCount() - 1; i >= 0; i--) {
						getChildAt(i).offsetTopAndBottom(-topOffset);
					}
					if (lastPosition < mItemCount - 1) {
			...
			switch (mLayoutMode) {
				...
				default:
					if (childCount == 0) {
						...
						if (mSelectedPosition >= 0 && mSelectedPosition < mItemCount && oldSel != null) {
	...
	int lookForSelectablePositionAfter(int current, int position, boolean lookDown) {
		...
		current = Math.min(Math.max(current, -1), count - 1);
		if (lookDown) {
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		...
		boolean handled = false; // super.dispatchKeyEvent(event);
		if (!handled) {
	...
	boolean pageScroll(int direction) {
		...
		if (nextPage >= 0) {
			...
			if (position >= 0) {
				...
				mSpecificTop = paddingTop; // + getVerticalFadingEdgeLength();
	...
	private int getArrowScrollPreviewLength() {
		return Math.max(MIN_SCROLL_PREVIEW_PIXELS, 0 /*getVerticalFadingEdgeLength()*/);
	}
	...
	private int distanceToView(View descendant) {
		...
		mTempRect.offset(descendant.getLeft() - descendant.getScrollX(), descendant.getTop() - descendant.getScrollY());
	...
	private void scrollListItemsBy(int amount) {
		...
		for (int i = getChildCount() - 1; i >= 0; i--) {
			getChildAt(i).offsetTopAndBottom(amount);
		}
		...
		if (amount < 0) {
			...
			if (last.getBottom() < listBottom) {
				...
				for (int i = getChildCount() - 1; i >= 0; i--) {
					getChildAt(i).offsetTopAndBottom(listBottom - last.getBottom());
				}
			}
			...
			if (first.getTop() > listTop) {
				...
				for (int i = getChildCount() - 1; i >= 0; i--) {
					getChildAt(i).offsetTopAndBottom(listTop - first.getTop());
				}
			}
	...
	public boolean isOpaque() {
		...
		if (retValue) {
			...
			final int listTop = mListPadding != null ? mListPadding.top : paddingTop;
			View first = getChildAt(0);
			if (first == null || first.getTop() > listTop) {
				return false;
			}
			final int listBottom = getHeight() - (mListPadding != null ? mListPadding.bottom : paddingBottom);
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (drawDividers || drawOverscrollHeader || drawOverscrollFooter) {
			...
			bounds.right = getRight() - getLeft() - paddingRight;
	...
	protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
		...
		if (adapter != null && gainFocus && previouslyFocusedRect != null) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				otherRect.offset(other.getLeft() - other.getScrollX(), other.getTop() - other.getScrollY());
