...
public class Cam {
	...
	public static Cam fromIntInFrame(int argb, @NonNull Frame frame) {
		...
		float hue =
		    atanDegrees < 0
			? atanDegrees + 360.0f
		    : atanDegrees >= 360 ? atanDegrees - 360.0f
					 : atanDegrees;
		...
		float q =
		    4.0f / frame.getC() * (float)Math.sqrt(j / 100.0f) * (frame.getAw() + 4.0f) * frame.getFlRoot();
	...
	private static Cam fromJchInFrame(float j, float c, float h, Frame frame) {
		float q =
		    4.0f / frame.getC() * (float)Math.sqrt(j / 100.0) * (frame.getAw() + 4.0f) * frame.getFlRoot();
		float m = c * frame.getFlRoot();
	...
	public int viewed(@NonNull Frame frame) {
		...
		float rC = Math.signum(rA) * (100.0f / frame.getFl()) * (float)Math.pow(rCBase, 1.0 / 0.42);
		float gCBase = (float)Math.max(0, (27.13 * Math.abs(gA)) / (400.0 - Math.abs(gA)));
		float gC = Math.signum(gA) * (100.0f / frame.getFl()) * (float)Math.pow(gCBase, 1.0 / 0.42);
		float bCBase = (float)Math.max(0, (27.13 * Math.abs(bA)) / (400.0 - Math.abs(bA)));
		float bC = Math.signum(bA) * (100.0f / frame.getFl()) * (float)Math.pow(bCBase, 1.0 / 0.42);
